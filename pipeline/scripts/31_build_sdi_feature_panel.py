...
import pandas as pd
import duckdb
from utils import DB_PATH, OUTPUT_ROOT, PROJECT_ROOT
...
# dt_open: prefer the harvested per-entity identifiers — complete across ALL Luck entities,
# including the 1976+ entities dropped by the 2026-06-05 Luck slim (08b) — with luck_call_reports
# (pre-1976 core) as fallback. Without this, a fresh rebuild against slimmed Luck would regress
# log_age for the dropped entities. The harvest parquet lives outside the warehouse; if absent
# (e.g. a clean checkout) fall back to luck_call_reports alone.
_HARVEST = PROJECT_ROOT / "Technical" / "coverage_analysis" / "luck_entity_identifiers.parquet"
if _HARVEST.exists():
    dto = con.execute(f"""
      SELECT entity_id AS rssd_id, MIN(dt_open) AS dt_open FROM (
        SELECT entity_id, dt_open FROM read_parquet('{_HARVEST.as_posix()}')
          WHERE dt_open IS NOT NULL AND dt_open > 0
        UNION ALL
        SELECT entity_id, value AS dt_open FROM luck_call_reports
          WHERE variable_id='dt_open' AND value > 0
      ) GROUP BY entity_id
    """).fetchdf()
else:
    dto = con.execute("""
