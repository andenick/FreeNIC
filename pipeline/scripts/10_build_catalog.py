...
def build_filing_coverage(con):
    ...
    con.execute("""
        ...
        -- NCUA 5300 (credit unions; cu_number namespace, not RSSD)
        SELECT 'ncua', period_end,
               COUNT(DISTINCT cu_number), COUNT(DISTINCT account_code), COUNT(*)
        FROM ncua_5300
        GROUP BY period_end

        UNION ALL

        -- HMDA mortgage-lending summary (LEI namespace; annual)
        SELECT 'hmda', MAKE_DATE(activity_year, 12, 31),
               COUNT(DISTINCT lei), COUNT(DISTINCT loan_purpose), COUNT(*)
        FROM hmda_summary
        GROUP BY activity_year

        UNION ALL

        -- UBPR standardized analytical ratios (rssd x period x UBPR concept)
        SELECT 'ubpr', period_end,
               COUNT(DISTINCT rssd_id), COUNT(DISTINCT ubpr_code), COUNT(*)
        FROM ubpr_ratios
        GROUP BY period_end

        UNION ALL

        -- FR Y-15 systemic-risk / G-SIB indicators (rssd x year x RISK line item)
        SELECT 'y15', period_end,
               COUNT(DISTINCT rssd_id), COUNT(DISTINCT risk_code), COUNT(*)
        FROM y15_systemic_indicators
        GROUP BY period_end
    """)
