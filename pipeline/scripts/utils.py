...
INPUT_PATHS = {
    ...
    # C3: NCUA 5300 credit-union call reports (quarterly bulk ZIPs). SCOPE EXPANSION
    # beyond FDIC-insured/BHC universe -- credit unions, keyed by NCUA CU_NUMBER.
    # Ingested by 26_ingest_ncua.py.
    'ncua': INPUTS_DIR / 'ncua_5300',
    # C2: CFPB HMDA Data Browser API raw JSON cache (filers list + per-LEI
    # loan-purpose aggregations). Acquired + ingested by 35_ingest_hmda.py.
    # ADJACENT dataset (mortgage lending), NOT the call-report core; an
    # institution x year summary, NOT the full LAR.
    'hmda': INPUTS_DIR / 'hmda',
}
...
SOURCE_DESCRIPTIONS = {
    ...
    'ncua_5300': 'NCUA 5300 credit-union call reports (FS220 schedules melted to long; cu_number '
                 'namespace; SCOPE EXPANSION beyond FDIC-insured/BHC).',
    'sec_cik_crosswalk': 'SEC EDGAR bank/BHC CIK<->identity crosswalk (XBRL frames + submissions '
                         'SIC filter; built via data.sec.gov).',
    'hmda_summary': 'HMDA mortgage-lending institution x year summary (CFPB Data Browser API; LEI '
                    'namespace; adjacent to the call-report core, NOT the full LAR).',
    'nic_entity_identifiers': 'Authoritative Fed multi-regulator ID crosswalk from NIC attributes '
                              '(RSSD<->LEI/CUSIP/OCC/NCUA/OTS-thrift/ABA/FDIC-cert/tax); powers id_crosswalk.',
    'nic_attributes_ext': 'NIC attribute extension: Fed-direct geography (street/zip/county/url), '
                          'charter/regulatory codes, and status/type per RSSD (the ~30 cols beyond the '
                          'simplified institutions/institution_attributes load).',
    'ubpr_ratios': 'FFIEC UBPR (Uniform Bank Performance Report) standardized analytical ratios, long '
                   'form (rssd x period x UBPR concept); XBRL bulk from the FFIEC CDR.',
    'y15_systemic_indicators': 'FR Y-15 banking-organization systemic-risk / G-SIB indicators (long '
                               'form rssd x year x RISK line item); FFIEC NIC Y-15 annual snapshots.',
}
